...
List<Photo> parsePhotos(String responseBody) {
  ...
  final parsed = parsed1["msg"]["product"].cast<Map<String, dynamic>>();
...
class Orderx extends State<MyOrders> {
  void initState() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(context, "Product Orders"),
      body: FutureBuilder<List<Photo>>(
        future: fetchPhotos(http.Client()),
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            loginToast("No Product Found");
